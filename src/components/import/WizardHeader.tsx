...
export function WizardHeader({ title, showStep = false, onHomeClick }: WizardHeaderProps) {
  ...
  return (
    <header className="wizard-header sticky top-0 z-40 bg-card border-b border-border/60 shadow-[0_1px_3px_hsl(var(--foreground)/0.04)]">
      <div className="wizard-header-inner container mx-auto px-4 max-w-5xl h-20 relative flex items-center">
        {onHomeClick ? (
          <button
            type="button"
            onClick={onHomeClick}
          ...
          >
            <img src={pupilLogo} alt="pupil by seven education" className="wizard-header-logo h-10 w-auto" />
