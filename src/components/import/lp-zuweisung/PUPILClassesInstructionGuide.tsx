...
import pupilKlassen from '@/assets/pupil-anleitung-klassen.png';
import pupilKlassenExcel from '@/assets/pupil-anleitung-klassen-excel.png';

const STORAGE_KEY = 'pupil-classes-guide-open';
...
export function PUPILClassesInstructionGuide() {
      ...
      <Collapsible open={open} onOpenChange={handleToggle}>
        <CollapsibleTrigger asChild>
          <Button variant="ghost" className="w-full justify-start gap-2 text-sm font-medium text-primary hover:bg-primary/5 px-3 py-2 h-auto">
            ...
            Anleitung: Klassen-Export aus PUPIL
        ...
        <CollapsibleContent className="px-3 pb-4 space-y-5 pt-2">
          ...
          <div className="space-y-2">
            ...
            <div className="space-y-1.5 text-sm text-muted-foreground">
              ...
              <div className="flex items-start gap-2">
                <StepNumber n={2} />
                <span><strong className="text-foreground">Schulen/Klassen/Gruppen</strong> anklicken</span>
              </div>
              <div className="flex items-start gap-2">
                <StepNumber n={3} />
                <span><strong className="text-foreground">Klassen</strong> auswählen</span>
              </div>
            </div>
            <div className="relative group w-fit">
              <img
                src={pupilKlassen}
                alt="PUPIL: Klassen-Ansicht"
                className="max-h-[200px] object-contain cursor-pointer rounded-lg border hover:shadow-md transition-shadow"
                onClick={() => setLightboxSrc(pupilKlassen)}
              />
          ...
          <div className="space-y-2">
            ...
            <div className="space-y-1.5 text-sm text-muted-foreground">
              <div className="flex items-start gap-2">
                <StepNumber n={4} />
                <span>Gesamte Klassentabelle mit Kopfzeile bis ganz nach unten markieren</span>
              </div>
              <div className="flex items-start gap-2">
                <StepNumber n={5} />
                <span>Mit Rechtsklick oder <kbd className="px-1.5 py-0.5 rounded bg-muted text-foreground text-xs font-mono">Ctrl+C</kbd> kopieren</span>
              </div>
              <div className="flex items-start gap-2">
                <StepNumber n={6} />
                <span>In leere Excel-Tabelle einfügen und <strong className="text-foreground">Excel speichern</strong></span>
              </div>
            </div>
            <div className="relative group w-fit">
              <img
                src={pupilKlassenExcel}
                alt="PUPIL: Klassen in Excel eingefügt"
                className="max-h-[200px] object-contain cursor-pointer rounded-lg border hover:shadow-md transition-shadow"
                onClick={() => setLightboxSrc(pupilKlassenExcel)}
              />
              <span className="absolute bottom-2 right-2 bg-background/80 backdrop-blur-sm text-xs px-2 py-1 rounded border opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none">
                🔍 Klicken zum Vergrössern
              </span>
